package org.example.Units;

import java.util.Random;

public class Sniper extends Man{

    public Sniper(String name, int x, int y, int health, int damage, int armor, int speed, int attack, int shots, int ammunition, int mana, int delivery) {
        super(name, x, y, health, damage, armor, speed, attack, shots, ammunition, mana, delivery);
    }

    public Sniper() {
        super(String.valueOf(Names.values()[new Random().nextInt(0,Names.values().length-1)]),0,0,15,new Random().nextInt(8,10),10,9,12,32,0,0,0);
        this.ammunition = 32;
    }



    public String toString() {
        return "Sniper\t\t{" +
                ANSI_YELLOW+"Имя: '" + name + '\'' +ANSI_RESET+
                ANSI_RED+",\t\t Здоровье: " + health +ANSI_RESET+
                ANSI_PURPLE+", Урон: " + damage +ANSI_RESET+
                ANSI_BLUE+", Броня: " + armor +ANSI_RESET+
                ", Скорость: " + speed +
                ", Атака: " + attack +
                ", Выстрелы: " + shots +
                ", Мана: " + mana +
                ", доставка: " + delivery +
                '}';
    }

    @Override
    public int step(int a) {
        return a;
    }

    @Override
    public String getInfo() {
        return ANSI_GREEN+"Я снайпер!"+ANSI_RESET;
    }
}
